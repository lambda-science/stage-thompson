...
for index, row in Error_file.iloc[:,:].iterrows():
    ...
    if prot_HumanRef == []:
        ...
        prot_trial = []
        for i in range(3):
            f = open("../raw/correction-pairwise/fasta/"+str(index)+"_"+peptide_Ref[0]+"_"+str(i)+".fasta", "w")
            ...
            mafft_cline = MafftCommandline("/biolo/mafft/inst/bin/mafft", 
                input="../raw/correction-pairwise/fasta/"+str(index)+"_"+peptide_Ref[0]+"_"+str(i)+".fasta", 
                op=5.0, ep=10.0)
