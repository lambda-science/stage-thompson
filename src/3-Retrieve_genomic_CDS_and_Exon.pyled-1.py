...
def makeAsyncEnsemblSeqRequest(ID_file, type_request):
    ...
    for i in range(0, 9960, 50):
        try:
            params.append({"ids": ID_file.iloc[i:i+50,1].tolist(), "type":type_request})
        except:
            params.append({"ids": ID_file.iloc[i:,1].tolist(), "type":type_request})
    ...
    for response in all_response:
        ...
        try:
            for entry in response.json():
                f.write(">"+entry['query']+" "+str(ID_file.iloc[indice*50+j,0]) + type_request +"\n")
                f.write(entry['seq']+"\n")
                j = j+1
        except:
            pass
